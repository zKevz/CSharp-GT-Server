...
namespace GTServer.Resources
{
    ...
    {
        public static async Task TakeDroppedItems(this ENetPeer peer,int netID, int x, int y,int item)
        {
            try
            {
                foreach (var a in Peers.Where(x => x.State == ENetPeerState.Connected && x.InWorld(peer)))
                {
                    ...
                    await SendPacketRaw(4, raw, 56, 0, a);
                }
                var world = peer.GetWorld();
                int i = x / 32, j = y / 32;
                int s = i + j * world.Width;
                foreach (var v in world.Items[s].DroppedItems)
                {
                    if (v.ItemID == 112)
                    {
                        (peer.Data as Player).Gems += v.ItemCount;
                    }
                    else
                    {
                        var invitem = new InventoryItems
                        {
                            ItemCount = (byte)v.ItemCount,
                            ItemID = v.ItemID
                        };
                        (peer.Data as Player).PlayerInventory.Items =
                            (peer.Data as Player).PlayerInventory.Items.Append(invitem).ToArray();
                        await SendInventory(peer, (peer.Data as Player).PlayerInventory);
                    }
                    world.Items[s].DroppedItems.Remove(v);
                }
            }
            catch
            {
        ...
        public static async Task SendDrop(this ENetPeer peer, int netID, int x, int y, int item, int count, byte specialEffect)
        {
            if (item >= 9000) return;
            if (item < 0) return;
        ...
        public static void SendLog(this ENetPeer peer, MethodBase method)
        {
            SendDateTime();
            Console.Write("Packet " + method.Name + " sent to the peer : ");
            Console.ForegroundColor = ConsoleColor.Blue;
            Console.Write((peer.Data as Player).TankIDName);
            Console.ForegroundColor = ConsoleColor.White;
            Console.Write(" with IP : ");
            Console.ForegroundColor = ConsoleColor.Blue;
            Console.WriteLine(peer.RemoteEndPoint.Address);
            Console.ForegroundColor = ConsoleColor.White;
        }
        public static async Task SendClothes(this ENetPeer peer)
        {
            try
            {
                ...
                if (UseLog)
                {
                    peer.SendLog(MethodBase.GetCurrentMethod());
                }
        ...
        public static async Task SendRoulette(this ENetPeer peer)
        {
            ...
            catch(Exception e)
            {
                if (UseLog)
                {
                    peer.SendLogError(MethodBase.GetCurrentMethod(), e);
                }
            }
        }
        public static void SendLogError(this ENetPeer peer, MethodBase method,Exception e)
        {
            SendDateTime();
            Console.ForegroundColor = ConsoleColor.Red;
            Console.Write("Packet " + method.Name + " FAILED TO sent to the peer : ");
            Console.ForegroundColor = ConsoleColor.Blue;
            Console.Write((peer.Data as Player).TankIDName);
            Console.ForegroundColor = ConsoleColor.Red;
            Console.Write(" with IP : ");
            Console.ForegroundColor = ConsoleColor.Blue;
            Console.Write(peer.RemoteEndPoint.Address);
            Console.ForegroundColor = ConsoleColor.White;
        ...
        public static async Task JoinWorld(this ENetPeer peer, string act, int x2, int y2)
        {
            try
            {
                ...
                for (int j = 0; j < info.Width * info.Height; j++)
                {
                    ...
                    foreach(var a in info.Items[j].DroppedItems)
                    {
                        await peer.SendDrop(-1, j % info.Width * 32, j / info.Width * 32, a.ItemID, a.ItemCount, 0);
                    }
                }
            ...
            catch (Exception ee)
            {
                if (UseErrorLog)
                {
                    peer.SendLogError(MethodBase.GetCurrentMethod(),ee);
                }
                int e = 0;
                if (e == 1)
                {
